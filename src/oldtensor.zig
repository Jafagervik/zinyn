...
pub fn Tensor(comptime T: type) type {
    return struct {
        ...
        // pub fn backward(self: *Self, grad: ?*Tensor) !void {
...
test "Set all" {
    ...
    const allocator = testing.allocator;
    var tensor = try TF32.ones(allocator, &[_]u32{ 2, 3, 4 });
    defer tensor.deinit();

    try testing.expectEqual(1.0, tensor.getFirst());

    tensor.setAll(5.0);

    try testing.expectEqual(5.0, tensor.getFirst());
}
